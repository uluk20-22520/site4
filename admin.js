...
const defaultUsers = [
  ...
  { username: 'viewer', password: 'viewer123', role: 'viewer', name: 'Менеджер' }
];
...
function renderDashboard() {
  ...
  document.getElementById('dashboardCards').innerHTML = `
    ...
    <div class="panel dashboard-card glow-border"><span class="muted">Хранение</span><span class="value">${settings.useCloud ? 'Supabase' : 'Локально'}</span></div>
  `;
}
...
async function getLeadSourceLabel() {
  ...
  if (settings.useCloud) {
    return 'Заявки загружены из облачного хранилища.';
  }
  return 'Заявки доступны в текущем хранилище.';
}
...
async function refreshLeadData() {
  ...
  try {
    ...
    showToast('Не удалось загрузить заявки. Показана доступная копия.');
  }
